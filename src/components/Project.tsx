...
const Project = () => {
  return (
    <div className="flex flex-col border-0 border-gray-600 items-center gap-12 lg:gap-20 py-8 md:px-16 rounded-2xl" id="project">
      <motion.h1 className="text-4xl font-bold" initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.5, delay: 0.5 }}>
        Best Projects
      </motion.h1>
      {projectData.map((project, index) => (
        <div key={index} className={`flex ${index / 2 === 0 ? "flex-col md:flex-row" : "flex-col md:flex-row-reverse"} border-0 border-green-500 gap-4 md:gap-16`}>
          <motion.div className="flex flex-1 flex-col gap-2" initial={index / 2 === 0 ? { x: -50, opacity: 0 } : { x: 50, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} transition={{ duration: 0.5, delay: 0.5 }}>
          ...
          <motion.div className="flex flex-1" initial={index / 2 === 0 ? { x: 50, opacity: 0 } : { x: -50, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} transition={{ duration: 0.5, delay: 0.5 }}>
